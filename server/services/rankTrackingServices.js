...
export async function rankTracker(keyword, targetDomain) {
  ...
  try {
    ...
    let found = null;
    const allResults = [];
    const cleanTarget = targetDomain.replace("www.", "").toLowerCase(); // ✅ FIXED: toLowerCase()
        ...
        try {
          ...
          pageResults = await page.evaluate(() => 
            Array.from(document.querySelectorAll("h3")).map((h3) => {
              ...
              for (let j = 0; j < 6 && c; j++, c = c.parentElement) { // ✅ FIXED: loop syntax
                const txt = c.innerText || "";
                const h3Text = h3.innerText || ""; // ✅ FIXED: added null check
                if (txt.length > h3Text.length + 50) {
                  const lines = txt.split("\n");
                  const foundLine = lines.find((l) => 
                    l.length > 30 && !l.includes(h3Text.substring(0, 20))
                  );
                  s = (foundLine || "").trim().substring(0, 300);
                  if (s) break;
                }
              }
